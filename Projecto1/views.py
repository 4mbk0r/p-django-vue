...
cred = credentials.Certificate(
    './chatprueba-b6c18-firebase-adminsdk-u82ob-19a9ace74c.json')
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = authenticate(username=request.POST.get(
                "nick"), password=request.POST.get("clave"))
            login(request, user)
            return redirect('/bienvenido/', foo='bar')
        else:
            print(form.errors)
...
def registar(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            dbf.collection(u'usuarios').document(
                u''+request.POST.get("nick")).set(data)
            user = user = User.objects.create_user(username=request.POST.get(
                "nick"), email=request.POST.get("correo"), password=request.POST.get("clave"))
            user.save()
            login(request, user)
...
def csrf_failure(request, reason=""):
    print(request)
    doc_pantilla = open(
        "E:/service worker/chatbot_django/Projecto1/Projecto1/template/defauld.html")
